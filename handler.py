...
def _get_params(body: dict) -> tuple[float, float]:
    ...
    if raw_sd is None:
        raise ValueError("missing required parameter 'sd' (payload field or SD env var)")
    if raw_uwp is None:
        raise ValueError("missing required parameter 'unit_win_prob' (payload field or UNIT_WIN_PROB env var)")
...
def handler(event, context):
    ...
    if results is None:
        return {"statusCode": 400, "body": json.dumps({"error": "payload must include 'results' array"})}
